...
for($i = 0; $i < sizeOf($newLine); $i++){
	$val = array();
	$pos1 = strpos($newLine[$i], ' ');

	array_push($val, substr($newLine[$i],0, $pos1));

	$pos2 = strpos($newLine[$i], '(');
	$pos3 = strpos($newLine[$i], ')');

	array_push($val, substr($newLine[$i],$pos2 + 1, $pos3 - $pos2 - 1));

	if(strpos($newLine[$i], '>') > -1){
		$workString = substr($newLine[$i], strpos($newLine[$i], '>') +2);
		array_push($val, explode(", ", $workString));
	}
	
	array_push($smallArray, $val);
}
echo getCost("mkxke", $smallArray);
function getCost($name,$arr){
	$cost = 0;
	for($i = 0; $i < sizeOf($arr); $i++){
		if($name == $arr[$i][0]){
			$cost += $arr[$i][1];
			$costArray = array();
			for($j = 0; $j < sizeOf($arr[$i][2]); $j++){
				array_push($costArray,getCost($arr[$i][2][$j], $arr));
			}
			$var = checkCost($costArray);
...
function checkCost($array){
	for ($j = 0; $j < sizeOf($array); $j++){
		for ($k = 0; $k < sizeOf($array); $k++){
			...
			if($array[$j] != $array[$k]){
				if($array[$j] != $array[($k + 1) % sizeOf($array)]){
					echo $array[$j] - $array[$k];
					echo "\n Were Done!!!!";
					return $j;
				}
				else {
